...
class Context {
  ...
  next() {
    return this.numbers.shift().call()
  }
...
export class WordProblem {
  ...
  answer() {
    if (!VALID_PATTERN.test(this.question)) {
      throw new ArgumentError()
    }

    let remainder = this.question.substring(
      PREFIXES.find(prefix => this.question.indexOf(prefix) === 0).length,
    ).trimLeft()
